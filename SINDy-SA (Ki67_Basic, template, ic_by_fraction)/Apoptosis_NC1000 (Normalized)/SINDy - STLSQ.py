...
def true_model(X, t, r01, r10, d, rA):
    Ki67_neg, Ki67_pos, apoptotic = X
    dXdt = [-r01*Ki67_neg + 2.0*r10*Ki67_pos - d*Ki67_neg, 
    	r01*Ki67_neg - r10*Ki67_pos - d*Ki67_pos,
    	d*Ki67_neg + d*Ki67_pos - rA*apoptotic
    ]
    return dXdt
...
t = data["times"][::step]
X = np.stack((data["Ki67_negative_mean"][::step], data["Ki67_positive_mean"][::step], data["apoptotic_mean"][::step]), axis = -1)

data_std = np.genfromtxt("data_std.csv", dtype = float, delimiter = ',', names = True)
X_std = np.stack((data_std["Ki67_negative_std"][::step], data_std["Ki67_positive_std"][::step], data_std["apoptotic_std"][::step]), axis = -1)

# norm = np.array([np.max(X[:,0]) - np.min(X[:,0]), np.max(X[:,1]) - np.min(X[:,1]), np.max(X[:,2]) - np.min(X[:,2])])
...
X_test = np.stack((data["Ki67_negative_mean"][::step], data["Ki67_positive_mean"][::step], data["apoptotic_mean"][::step]), axis = -1)

# norm = np.array([np.max(X_test[:,0]) - np.min(X_test[:,0]), np.max(X_test[:,1]) - np.min(X_test[:,1]), np.max(X_test[:,2]) - np.min(X_test[:,2])])
...
d = 0.000053167
rA = 0.001937984
true_solution = odeint(true_model, X0_test, t_test, args=(r01, r10, d, rA))
		...
		for stlsq_alpha in stlsq_alphas:
			...
			optimizer = ps.STLSQ(
				...
				non_physical_features = [['1', 'apoptotic', 'Ki67_neg apoptotic', 'Ki67_pos apoptotic', 'apoptotic^2'],
					['1', 'apoptotic', 'Ki67_neg apoptotic', 'Ki67_pos apoptotic', 'apoptotic^2'],
					['1']
				]
			)
			# Feature names = ['1', 'Ki67_neg', 'Ki67_pos', 'apoptotic', 'Ki67_neg^2', 'Ki67_neg Ki67_pos', 'Ki67_neg apoptotic', 'Ki67_pos^2', 'Ki67_pos apoptotic', 'apoptotic^2']
			...
			model = ps.SINDy(
				...
				optimizer = optimizer,
				feature_names = ["Ki67_neg", "Ki67_pos", "apoptotic"]
			)
			model.fit(X, t = t)
