...
X_test = np.delete(X_test, slice(ic_size), 0)
t_test = np.delete(t_test, slice(ic_size), None)
...
r01 = 0.003631082
r10 = 0.001075269
true_solution = odeint(true_model, X0_plot, t_plot, args=(r01, r10))
...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(1, 4):
		...
		ax.plot(t_plot, true_solution[:,0], "b:", label = r"True Ki67$^{-}(t)$", alpha = 1.0, linewidth = 1)
		ax.plot(t_plot, true_solution[:,1], "g:", label = r"True Ki67$^{+}(t)$", alpha = 1.0, linewidth = 1)
