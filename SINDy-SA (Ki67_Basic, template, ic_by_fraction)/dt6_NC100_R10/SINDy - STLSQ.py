...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(1, 4):
		...
		ax.errorbar(t_test, X_test[:,0], yerr = std_factor*X_std[:,0], fmt = 'ko', label = r"Data Ki67$^{-}(t)$", capsize = 2.0, alpha = 0.3, markersize = 3)
		ax.errorbar(t_test, X_test[:,1], yerr = std_factor*X_std[:,1], fmt = 'k^', label = r"Data Ki67$^{+}(t)$", capsize = 2.0, alpha = 0.3, markersize = 3)
