...
t = data["times"][::step]
X = np.stack((data["G0G1_phase_mean"][::step], data["S_phase_mean"][::step], data["G2M_phase_mean"][::step]), axis = -1)

data_std = np.genfromtxt("data_std.csv", dtype = float, delimiter = ',', names = True)
X_std = np.stack((data_std["G0G1_phase_std"][::step], data_std["S_phase_std"][::step], data_std["G2M_phase_std"][::step]), axis = -1)
...
X_test = np.stack((data["G0G1_phase_mean"][::step], data["S_phase_mean"][::step], data["G2M_phase_mean"][::step]), axis = -1)
...
r20 = 0.003333333
r12 = 0.002083333
true_solution = odeint(true_model, X0_test, t_test, args=(r01, r20, r12))
...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(1, 4):
		...
		# ax.plot(t_test, X_test[:,2], "ks", label = r"Data G$_2$ M$(t)$", alpha = 0.5, markersize = 3)
		...
		ax.errorbar(t_test, X_test[:,2], yerr = std_factor*X_std[:,2], fmt = 'ks', label = r"Data G$_2$ M$(t)$", capsize = 2.0, alpha = 0.3, markersize = 3)
		...
		ax.plot(t_test, true_solution[:,2], "r:", label = r"True G$_2$ M$(t)$", alpha = 1.0, linewidth = 1)
		...
		ax.plot(t_test, simulation[:,2], "r", label = r"SINDy-SA G$_2$ M$(t)$", alpha = 1.0, linewidth = 1)
