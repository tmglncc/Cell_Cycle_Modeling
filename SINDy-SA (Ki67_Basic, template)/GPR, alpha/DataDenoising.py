...
class DataDenoising:
	...
	def gaussian_process_regression(self, kernel = RBF(), n_restarts_optimizer = 10, alpha = 1.0e-10, t_pred = None):
		...
		for j in range(X_gpr_mean.shape[1]):
			model = GaussianProcessRegressor(kernel = kernel, n_restarts_optimizer = n_restarts_optimizer, alpha = alpha[:,j], normalize_y = np.max(self.X[:,j]))
	...
	def plot_gpr(self, kernels = None, n_restarts_optimizers = None, alphas = None, kernel_strings = None, set_id = ""):
		if kernels is not None and n_restarts_optimizers is not None and alphas is not None:
			for i, feature_name in enumerate(self.feature_names):
				...
				for j, kernel in enumerate(kernels):
					for n_restarts_optimizer in n_restarts_optimizers:
						for alpha in alphas:
							...
							ax.plot(self.t, X_gpr_mean[:,i], label = "GPR(" + kernel_strings[j] + ", " + str(n_restarts_optimizer) + ")", alpha = 1.0, linewidth = 1)
				...
				ax.legend()
				# fig.show()
				plt.savefig(os.path.join("output", "GPR" + str(set_id) + "_" + self.feature_names[i] + ".png"), bbox_inches = 'tight')
				plt.close()
