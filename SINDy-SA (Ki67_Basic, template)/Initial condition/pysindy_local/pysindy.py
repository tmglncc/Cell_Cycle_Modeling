...
from .utils import symbolic_equations
...
class SINDy(BaseEstimator):
    ...
    def fit(
        ...
        optimizer = SINDyOptimizer(self.optimizer, unbias=unbias, feature_names=self.feature_names, symbolic_equations=self.symbolic_equations, data=x, time=t, get_feature_names=self.get_feature_names)
    ...
    def symbolic_equations(self, param_names=None, input_fmt=None):
        check_is_fitted(self, "model")
        if self.discrete_time:
        ...
        return symbolic_equations(
            self.model, input_features=base_feature_names, param_names=param_names, input_fmt=input_fmt
        )
    ...
    def coefficients(self, coef=None):
        """
        ...
        check_is_fitted(self, "model")
        if coef is not None:
            self.model.steps[-1][1].set_coefficients(coef)
        return self.model.steps[-1][1].coef_

    def get_X_dot_model(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].X_dot_model_

    def get_SSE(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].SSE_

    def get_SSE_data(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].SSE_data_

    def get_relative_error(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].relative_error_

    def get_Ftest(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].Ftest_

    def get_Ftest_data(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].Ftest_data_

    def get_mean(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].mean_

    def get_epsilon_std(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].epsilon_std_

    def get_num_eval(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].num_eval_

    def get_mu_star(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].mu_star_

    def get_sigma(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].sigma_

    def get_param_min(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].param_min_

    def get_history(self):
        check_is_fitted(self, "model")
        return self.model.steps[-1][1].history_
    ...
    def simulate(
        ...
        if self.discrete_time:
            ...
            if u is None or self.n_control_features_ == 0:
                ...
                # def input_fmt(string):
                #     return string.replace(" ", "*").replace("^", "**").replace("cos", "np.cos").replace("sin", "np.sin").replace("log", "np.log")

                # def create_rhs_func(param_expression, var_expression, model_expression, n_vars):
                #     function = """def rhs(X, t, """ + param_expression + """):
                #         import numpy as np
                #         """ + var_expression + """ = X
                ...
                # param_names = np.array(["c" + str(i) for i in range(len(self.model.steps[-1][1].ind_.flatten()))])
                # param_names = np.reshape(param_names, self.model.steps[-1][1].ind_.shape)
                # param_expression = ", ".join(param_names[self.model.steps[-1][1].ind_].flatten()) + ","

                # var_names = self.feature_names
                # var_expression = ", ".join(var_names)

                # symbolic_equations = self.symbolic_equations(param_names, input_fmt)
                # model_expression = ", ".join(symbolic_equations)

                # wrapper = {}
                # function = create_rhs_func(param_expression, var_expression, model_expression, len(var_names))
                # exec(function, wrapper)

                # return integrator(wrapper['rhs'], x0, t, args=tuple(self.model.steps[-1][1].coef_[self.model.steps[-1][1].ind_].flatten()), **integrator_kws)
            ...
            solution = integrator(rhs, [t[0], t[-1]], x0, method='LSODA', t_eval=t, **integrator_kws)
            solution = transpose(solution.y)
            return solution
