...
t = data["times"][::step]
X = np.stack((data["Ki67_negative_mean"][::step], data["Ki67_positive_premitotic_mean"][::step], data["Ki67_positive_postmitotic_mean"][::step]), axis = -1)

data_std = np.genfromtxt("data_std.csv", dtype = float, delimiter = ',', names = True)
X_std = np.stack((data_std["Ki67_negative_std"][::step], data_std["Ki67_positive_premitotic_std"][::step], data_std["Ki67_positive_postmitotic_std"][::step]), axis = -1)
...
X_test = np.stack((data["Ki67_negative_mean"][::step], data["Ki67_positive_premitotic_mean"][::step], data["Ki67_positive_postmitotic_mean"][::step]), axis = -1)
...
r12 = 0.001282051
r20 = 0.006666667
true_solution = odeint(true_model, X0_test, t_test, args=(r01, r10, r12, r20))
...
for poly_degree in poly_degrees:
	for fourier_nfreq in fourier_nfreqs:
		for stlsq_alpha in stlsq_alphas:
			...
			# 'Ki67_neg^2', 'Ki67_neg Ki67_pos_pre', 'Ki67_neg Ki67_pos_post', 'Ki67_pos_pre^2', 'Ki67_pos_pre Ki67_pos_post', 'Ki67_pos_post^2']
			...
			model = ps.SINDy(
				...
				optimizer = optimizer,
				feature_names = ["Ki67_neg", "Ki67_pos_pre", "Ki67_pos_post"]
			)
			model.fit(X, t = t)
...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(1, 4):
		...
		# ax.plot(t_test, X_test[:,1], "k^", label = r"Data Ki67$^{+}(t)$ (pre)", alpha = 0.5, markersize = 3)
		# ax.plot(t_test, X_test[:,2], "ks", label = r"Data Ki67$^{+}(t)$ (post)", alpha = 0.5, markersize = 3)
		ax.errorbar(t_test, X_test[:,0], yerr = std_factor*X_std[:,0], fmt = 'ko', label = r"Data Ki67$^{-}(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		ax.errorbar(t_test, X_test[:,1], yerr = std_factor*X_std[:,1], fmt = 'k^', label = r"Data Ki67$^{+}(t)$ (pre)", capsize = 2.0, alpha = 0.5, markersize = 3)
		ax.errorbar(t_test, X_test[:,2], yerr = std_factor*X_std[:,2], fmt = 'ks', label = r"Data Ki67$^{+}(t)$ (post)", capsize = 2.0, alpha = 0.5, markersize = 3)
