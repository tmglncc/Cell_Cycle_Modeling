...
class Fitter(object):
    ...
    def _optimizer(self, func, x0, args, disp):
        def _differential_evolution(bounds_perc = 0.99, number_of_restarts = 20,
        strategy = 'best1bin', maxiter = 500, popsize_factor = 5, tol = 1.0e-10,
        mutation = (0.5, 1), recombination = 0.7, polish = True, disp = False,
        seed = 7):
            print("*** Using Differential Evolution method ***\n")

            coef = self.single_dist._fitstart(self._data)
            bounds = []
            ...
            popsize = popsize_factor*len(coef)
            options = {
            ...
            for restart in range(1, number_of_restarts+1):
                ...
                if restart == 1:
                    result = differential_evolution(func, bounds, args=args, **options)
                else:
                    result = differential_evolution(func, bounds, args=args, x0=result.x, **options)
        ...
        return _differential_evolution()

    def _fit_single_distribution(self, distribution, loc=None, scale=None):
        try:
            # need a subprocess to check time it takes. If too long, skip it
            dist = eval("scipy.stats." + distribution)
            self.single_dist = dist
            ...
            if loc is None and scale is None:
                param = dist.fit(self._data, optimizer = self._optimizer)
            elif loc is None:
                param = dist.fit(self._data, scale = scale, optimizer = self._optimizer)
            elif scale is None:
                param = dist.fit(self._data, loc = loc, optimizer = self._optimizer)
            else:
                param = dist.fit(self._data, loc = loc, scale = scale, optimizer = self._optimizer)
    ...
    def fit(self, loc=None, scale=None, progress=False, n_jobs=-1, max_workers=-1):
        ...
        for i, dist in enumerate(self.distributions):
            if loc is None and scale is None:
                ...
                self._fit_single_distribution(dist, scale[i])
            elif scale is None:
                self._fit_single_distribution(dist, loc[i])
            else:
                self._fit_single_distribution(dist, loc[i], scale[i])
