...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(0, 1):
		...
		ax2 = ax1.twinx()
		ax1.plot(t_test, X_test[:,0], "bo", label = r"Data live$(t)$", alpha = 0.5, markersize = 3)
		ax2.plot(t_test, X_test[:,1], "g^", label = r"Data apoptotic$(t)$", alpha = 0.5, markersize = 3)
		# ax1.errorbar(t_test, X_test[:,0], yerr = std_factor*X_std[:,0], fmt = 'bo', label = r"Data live$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		# ax2.errorbar(t_test, X_test[:,1], yerr = std_factor*X_std[:,1], fmt = 'g^', label = r"Data apoptotic$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		ax1.plot(t_test, true_solution[:,0], "b:", label = r"True live$(t)$", alpha = 1.0, linewidth = 1)
		ax2.plot(t_test, true_solution[:,1], "g:", label = r"True apoptotic$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, simulation[:,0], "b", label = r"SINDy-SA live$(t)$", alpha = 1.0, linewidth = 1)
		ax2.plot(t_test, simulation[:,1], "g", label = r"SINDy-SA apoptotic$(t)$", alpha = 1.0, linewidth = 1)
		...
		ax1.set_ylabel(r"Live$(t)$", color = "b")
		ax2.set_ylabel(r"Apoptotic$(t)$", color = "g")
