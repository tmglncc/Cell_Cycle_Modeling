...
def true_model(X, t, r01, r30, r12, r23, d, rA):
        G0G1, S, G2, M, apoptotic = X
        dXdt = [-r01*G0G1 + 2.0*r30*M - d*G0G1,
            r01*G0G1 - r12*S - d*S,
            r12*S - r23*G2 - d*G2,
            r23*G2 - r30*M - d*M,
            d*G0G1 + d*S + d*G2 + d*M - rA*apoptotic
        ]
        return dXdt
...
t = data["times"][::step]
X = np.stack((data["G0G1_phase_mean"][::step], data["S_phase_mean"][::step], data["G2_phase_mean"][::step], data["M_phase_mean"][::step], data["apoptotic_mean"][::step]), axis = -1)

data_std = np.genfromtxt("data_std.csv", dtype = float, delimiter = ',', names = True)
X_std = np.stack((data_std["G0G1_phase_std"][::step], data_std["S_phase_std"][::step], data_std["G2_phase_std"][::step], data_std["M_phase_std"][::step], data_std["apoptotic_std"][::step]), axis = -1)
...
X_test = np.stack((data["G0G1_phase_mean"][::step], data["S_phase_mean"][::step], data["G2_phase_mean"][::step], data["M_phase_mean"][::step], data["apoptotic_mean"][::step]), axis = -1)
...
d = 0.000053167
rA = 0.001937984
true_solution = odeint(true_model, X0_test, t_test, args=(r01, r30, r12, r23, d, rA))
	...
	for fourier_nfreq in fourier_nfreqs:
		for stlsq_alpha in stlsq_alphas:
			...
			optimizer = ps.STLSQ(
				...
				non_physical_features = [['1', 'S', 'G2', 'G0G1 S', 'G0G1 G2', 'S^2', 'S G2', 'S M', 'G2^2', 'G2 M', 'apoptotic', 'G0G1 apoptotic', 'S apoptotic', 'G2 apoptotic', 'M apoptotic', 'apoptotic^2'],
					['1', 'G2', 'M', 'G0G1 G2', 'G0G1 M', 'S G2', 'S M', 'G2^2', 'G2 M', 'M^2', 'apoptotic', 'G0G1 apoptotic', 'S apoptotic', 'G2 apoptotic', 'M apoptotic', 'apoptotic^2'],
					['1', 'G0G1', 'M', 'G0G1^2', 'G0G1 S', 'G0G1 G2', 'G0G1 M', 'S M', 'G2 M', 'M^2', 'apoptotic', 'G0G1 apoptotic', 'S apoptotic', 'G2 apoptotic', 'M apoptotic', 'apoptotic^2'],
					['1', 'G0G1', 'S', 'G0G1^2', 'G0G1 S', 'G0G1 G2', 'G0G1 M', 'S^2', 'S G2', 'S M', 'apoptotic', 'G0G1 apoptotic', 'S apoptotic', 'G2 apoptotic', 'M apoptotic', 'apoptotic^2'],
					['1']
				]
			)
			# Feature names = ['1', 'G0G1', 'S', 'G2', 'M', 'apoptotic', 'G0G1^2', 'G0G1 S', 'G0G1 G2', 'G0G1 M', 'G0G1 apoptotic', 
			# 'S^2', 'S G2', 'S M', 'S apoptotic', 'G2^2', 'G2 M', 'G2 apoptotic', 'M^2', 'M apoptotic', 'apoptotic^2']
			...
			model = ps.SINDy(
				...
				optimizer = optimizer,
				feature_names = ["G0G1", "S", "G2", "M", "apoptotic"]
			)
			model.fit(X, t = t)
...
for model_id, model in enumerate(model_set):
	...
	for std_factor in range(0, 1):
		...
		ax2 = ax1.twinx()
		ax1.plot(t_test, X_test[:,0], "bo", label = r"Data G0G1$(t)$", alpha = 0.5, markersize = 3)
		ax1.plot(t_test, X_test[:,1], "g^", label = r"Data S$(t)$", alpha = 0.5, markersize = 3)
		ax1.plot(t_test, X_test[:,2], "rs", label = r"Data G2$(t)$", alpha = 0.5, markersize = 3)
		ax1.plot(t_test, X_test[:,3], "mD", label = r"Data M$(t)$", alpha = 0.5, markersize = 3)
		ax2.plot(t_test, X_test[:,4], "yp", label = r"Data apoptotic$(t)$", alpha = 0.5, markersize = 3)
		# ax1.errorbar(t_test, X_test[:,0], yerr = std_factor*X_std[:,0], fmt = 'bo', label = r"Data G0G1$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		# ax1.errorbar(t_test, X_test[:,1], yerr = std_factor*X_std[:,1], fmt = 'g^', label = r"Data S$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		# ax1.errorbar(t_test, X_test[:,2], yerr = std_factor*X_std[:,2], fmt = 'rs', label = r"Data G2$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		# ax1.errorbar(t_test, X_test[:,3], yerr = std_factor*X_std[:,3], fmt = 'mD', label = r"Data M$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		# ax2.errorbar(t_test, X_test[:,4], yerr = std_factor*X_std[:,4], fmt = 'yp', label = r"Data apoptotic$(t)$", capsize = 2.0, alpha = 0.5, markersize = 3)
		ax1.plot(t_test, true_solution[:,0], "b:", label = r"True G0G1$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, true_solution[:,1], "g:", label = r"True S$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, true_solution[:,2], "r:", label = r"True G2$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, true_solution[:,3], "m:", label = r"True M$(t)$", alpha = 1.0, linewidth = 1)
		ax2.plot(t_test, true_solution[:,4], "y:", label = r"True apoptotic$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, simulation[:,0], "b", label = r"SINDy-SA G0G1$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, simulation[:,1], "g", label = r"SINDy-SA S$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, simulation[:,2], "r", label = r"SINDy-SA G2$(t)$", alpha = 1.0, linewidth = 1)
		ax1.plot(t_test, simulation[:,3], "m", label = r"SINDy-SA M$(t)$", alpha = 1.0, linewidth = 1)
		ax2.plot(t_test, simulation[:,4], "y", label = r"SINDy-SA apoptotic$(t)$", alpha = 1.0, linewidth = 1)
		if calibration_mode == "Bayes":
			...
			ax1.fill_between(t, simulation_min[:,3], simulation_max[:,3], color = "m", alpha = 0.4)
			ax2.fill_between(t, simulation_min[:,4], simulation_max[:,4], color = "y", alpha = 0.4)
		ax1.set_xlabel(r"Time $t$")
		ax1.set_ylabel(r"G0G1$(t)$, S$(t)$, G2$(t)$, M$(t)$", color = 'k')
